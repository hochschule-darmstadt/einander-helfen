...
for placeNamePostcode in placeNamePostcodeArray:
	...
	if location is None:
		...
		resp = requests.get('https://nominatim.openstreetmap.org/search/?city=%s&country=Germany&postcalcode=%s&state=%s&format=json' % (placeNamePostcode.place_name, placeNamePostcode.postcode, placeNamePostcode.state))
...
for named_place in placeNameArray:
	...
	if len(places) > 1:
		...
		resp = requests.get('https://nominatim.openstreetmap.org/details?osmtype=R&osmid=%s&format=json' % named_place.osmid)
		...
		if 'centroid' in respJson:
			...
			print(named_place.place_name + " (alle), area: " + str(calculated_area) + ", lat: " + str(centro_lat) + ", lon: " + str(centro_lon))
